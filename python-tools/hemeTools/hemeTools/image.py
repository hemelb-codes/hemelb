...
class Image(object):
    ...
    def old__init__(self, filename):
        ...
        sortedIndices = np.argsort(
            tempPixels["index"][:, 0] << 16 + tempPixels["index"][:, 1]
        )
    ...
    def __eq__(self, other):
        ...
        if not np.alltrue(
            self.pixels.view(dtype=np.uint32) == other.pixels.view(dtype=np.uint32)
        ):
            return False
        ...
        for attr in ("r", "g", "b"):
            ...
            if np.min(d) < (128 - tol) or np.max(d) > (128 + tol):
                ...
                print(
                    str(np.min(d) - 128)
                    + " between "
                    + str(self.pixels[minArg])
                    + " and "
                    + str(other.pixels[minArg])
                )
                print("and")
                print(
                    str(np.max(d) - 128)
                    + " between "
                    + str(self.pixels[maxArg])
                    + " and "
                    + str(other.pixels[maxArg])
                )
                return False
            continue
