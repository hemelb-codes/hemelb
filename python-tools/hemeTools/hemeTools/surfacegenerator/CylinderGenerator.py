...
if __name__ == "__main__":
    ...
    p.add_argument(
        "resolution", type=int, help="number of segments around circumference"
    )
    p.add_argument("outfile", help="output file name")
    p.add_argument(
        "rotx",
        type=float,
        help="rotation about x-axis in degrees",
        nargs="?",
        default=0.0,
    )
    p.add_argument(
        "roty",
        type=float,
        help="rotation about y-axis in degrees",
        nargs="?",
        default=0.0,
    )
    ...
    matx = np.array(
        [
            [1.0, 0.0, 0.0],
            [0.0, np.cos(rotx), -np.sin(rotx)],
            [0.0, np.sin(rotx), np.cos(rotx)],
        ]
    )
    ...
    maty = np.array(
        [
            [np.cos(roty), 0.0, np.sin(roty)],
            [0.0, 1.0, 0.0],
            [-np.sin(roty), 0.0, np.cos(roty)],
        ]
    )
    ...
    CylinderGenerator(
        args.radius, args.length, args.resolution, args.outfile, direction
    )
