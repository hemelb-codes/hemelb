...
def cache(filename, *argPatterns, **kwargPatterns):
    ...
    def decorator(f):
        ...
        def cacher(*args, **kwargs):
            ...
            if len(argPatterns):
                cachefile += "." + ".".join(
                    [a % args[i] for i, a in enumerate(argPatterns)]
                )
                pass
            ...
            if len(keys):
                keys.sort()
                cachefile += "." + ".".join(
                    [kwargPatterns[k] % kwargs[k] for k in keys]
                )
                pass
            ...
            if os.path.exists(cachefile):
                ...
                ans = f(*args, **kwargs)
                cPickle.dump(ans, open(cachefile, "wb"), protocol=2)
                pass
            return ans
...
def processesFile(f):
    ...
    def cacher(infile):
        ...
        if os.path.exists(cachefile) and os.path.getmtime(cachefile) > os.path.getmtime(
            infile
        ):
