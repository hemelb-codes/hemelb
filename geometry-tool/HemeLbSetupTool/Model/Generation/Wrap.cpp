...
PYBIND11_MODULE(Generation, mod) {
  ...
  py::class_<DV>(mod, "DoubleVector")
    ...
    // get/set via []
    .def("__getitem__", [](DV const& self, int i) {
	return self[i];
      })
    .def("__setitem__", [](DV& self, int i, double v) {
	self[i] = v;
      })
    .def(py::self / double())
    ;
  ...
  auto gmy_gen = py::class_<GeometryGenerator>(mod, "GeometryGenerator")
    .def("GetOutputGeometryFile", &GeometryGenerator::GetOutputGeometryFile)
    .def("SetOutputGeometryFile", &GeometryGenerator::SetOutputGeometryFile)
  ...
  py::class_<PolyDataGenerator>(mod, "PolyDataGenerator", gmy_gen)
    .def(py::init<>())
    .def("SetSeedPointWorking", py::overload_cast<double, double, double>(&PolyDataGenerator::SetSeedPointWorking))
