...
struct Neighbours {
  ...
  using DoubleDisplacementArray = std::array<Vector, gmy::NumberOfDisplacements>;
  ...
  static constexpr DoubleDisplacementArray DoubleDisplacements = detail::i2d(gmy::Neighbourhood);
  // Index of inverses, i.e. Displacement[i] == -Displacement[Inverses[i]]
  static constexpr std::array<unsigned, gmy::NumberOfDisplacements> Inverses = detail::mk_inv(gmy::Neighbourhood);
};
#endif
