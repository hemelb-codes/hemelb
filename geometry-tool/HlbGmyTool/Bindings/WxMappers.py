        ...
        try:
            ...
            try:
                ...
                raise AttributeError(
                    "'%s' object has no property '%s' or Get%s/Set%s pair"
...
class WxWidgetEnabledMapper(NonObservingWxWidgetMapper):
    ...
    def __init__(
        self, widget, translator=QuickTranslator(setterHelper, lambda x: None)
    ):
        NonObservingWxWidgetMapper.__init__(
            self, widget, "Enabled", translator=translator
        )
        return
...
class WxListCtrlMapper(ListContentsDestMapper):
    ...
    HlbGmyTool.Bindings.ListController.ListContentsSourceMapper
