...
void visVisualiseString (float r, float g, float b, float x, float y, char *string, void *font)
{
  glColor3f (r, g, b);
  glRasterPos2f (x, y);
			  ...
			  float x_min, float y_min, float x_max)
{
  ...
  for (int i = 0; i <= 4; i++)
    {
      sprintf (string, "%.2f", threshold_min + 0.25 * i *(threshold_max - threshold_min));
      
      visVisualiseString (0.F, 0.F, 0.F, x_min + i * dx - 20.0F, y_min - 16.0F,
			  string, GLUT_BITMAP_HELVETICA_18);
    }
  visVisualiseString (0.F, 0.F, 0.F, x_max + 45, y_min - 16.0F,
		      unit, GLUT_BITMAP_HELVETICA_18);
}


void visVisualiseColourBar (int type, float x_min, float y_min, float x_max, float y_max)
{
  float dx = 0.25F * (x_max - x_min);
  float col[3], grey;
...
void SaveWindowImage (int pixels_x, int pixels_y, char *file_name)
{
  ...
  for (j = 0; j < pixels_y; j++)
	...
	{
	  *data_p = image_buffer[ i * 3     ]; data_p++;
	  *data_p = image_buffer[ i * 3 + 1 ]; data_p++;
	  *data_p = image_buffer[ i * 3 + 2 ]; data_p++;
	}
    }
  ...
  for (j = pixels_y - 1; j >= 0; j--)
    {
      fwrite (image_data + j * pixels_x * 3, 1, pixels_x * 3, ppm_image_file_ptr);
    }
...
void Display (void)
{
  ...
  for (n = 0; n < col_pixels; n++)
    {
      ...
      glColor3f (pixel_r[0] * (1.F / 255.F),
		 pixel_g[0] * (1.F / 255.F),
		 pixel_b[0] * (1.F / 255.F));
      glVertex2f (pixel_i, pixel_j + (pixels_y >> 1));
      
      glColor3f (pixel_r[1] * (1.F / 255.F),
		 pixel_g[1] * (1.F / 255.F),
		 pixel_b[1] * (1.F / 255.F));
      glVertex2f (pixel_i + (pixels_x >> 1), pixel_j + (pixels_y >> 1));
      
      glColor3f (pixel_r[2] * (1.F / 255.F),
		 pixel_g[2] * (1.F / 255.F),
		 pixel_b[2] * (1.F / 255.F));
      glVertex2f (pixel_i, pixel_j);
      
      glColor3f (pixel_r[3] * (1.F / 255.F),
		 pixel_g[3] * (1.F / 255.F),
		 pixel_b[3] * (1.F / 255.F));
  ...
  visVisualiseColourBar (RAINBOW,
			 pixels_x * 0.1F, pixels_y * 0.5F + 20.F,
  ...
  visVisualiseColourBar (RAINBOW,
			 pixels_x * 0.6F, pixels_y * 0.5F + 20.F,
  ...
  visVisualiseColourBar (palette_type,
			 pixels_x * 0.1F, 20.F,
			 pixels_x * 0.4F, 40.F);
  visVisualiseBarPars (pressure_unit, pressure_threshold_min, pressure_threshold_max,
		       pixels_x * 0.1F, 20.F,
		       pixels_x * 0.4F);
...
int main (int argc, char *argv[])
{
  ...
  image_data = (unsigned char *)malloc(sizeof(unsigned char) * pixels_x * pixels_y * 3);
  
  image_buffer = (unsigned char *)malloc(sizeof(unsigned char) * pixels_x * 3);
