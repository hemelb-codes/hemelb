/*
 * Created on 19-Jun-2008
 *
 * TODO To change the template for this generated file go to
 * Window - Preferences - Java - Code Style - Code Templates
 */
package uk.ac.ucl.chem.ccs.clinicalgui;
//import InsightToolkit.*;

/**
 * @author Konstantin Voevodski
 *
 * Failed attempt at using the itk library to handle DICOM files:
 * it is unclear how the methods documented in the C++ itk library are mapped
 * to the methods in the generated java "wrappings" 
 */
public class TestItkFunctionality {

	public static void main(String[] args) {
		/*System.out.println("DicomSliceRead Example");
		//itkImageFileReader reader = new itkImageFileReader();
	    //itkImageFileReaderUS2_Pointer reader = new itkImageFileReaderUS2_Pointer();
		itkImageFileReaderUS2_Pointer reader = itkImageFileReaderUS2.itkImageFileReaderUS2_New();
		//itkImageFileReaderUS2_Pointer reader = itkImageFileReaderUS2.itkImageFileReaderUS2_New();
		itkImageFileWriterUC2_Pointer writer = itkImageFileWriterUC2.itkImageFileWriterUC2_New();
		itkRescaleIntensityImageFilterUS2UC2_Pointer filter = itkRescaleIntensityImageFilterUS2UC2.itkRescaleIntensityImageFilterUS2UC2_New();
		itkDicomImageIO_Pointer dicomIO = itkDicomImageIO.itkDicomImageIO_New();	
		String inst = new String();
		//dicomIO.GetModality(inst);
		//System.out.println("Attrib: " + inst);
		
		reader.SetFileName( "/home/konstantin/file6.dcm" );
		writer.SetFileName( "/home/konstantin/file6.jpg" );
		filter.SetOutputMinimum( (short) 0 );
		filter.SetOutputMaximum( (short) 255 );
		reader.SetImageIO( (itkImageIOBase) dicomIO.GetPointer() );
		//filter.SetInput( reader.GetOutput() );
		//writer.SetInput( filter.GetOutput() );
*/

//		writer.Update();
	}
}
