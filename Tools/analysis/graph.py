...
class Graph(object):
    ...
    def specialise(self, *specialisations):
        """Return a copy of this graph, with the configuration
        modified by some additional dicts of dicts.
        """
    ...
    def prepare(self, results):
        ...
        self.curve_results = {
            key: list(group)
            for key, group in itertools.groupby(sorted(self.filtered_results, key=curve_key),
                                                curve_key)
        }
        ...
        self.curve_data = {
            curve: [ 
                [result.datum(independent) for independent in self.independent] + \
                [result.datum(dependent) for dependent in self.dependent] + \
                list(curve) \
    ...
    def write_data(self, csv_file):
        ...
        # Write a comment explaining what the groups between which
        # blank lines are inserted are.  This makes gnuplot not
        # join-the-dots across these breaks.
