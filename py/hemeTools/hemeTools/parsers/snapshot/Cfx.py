...
def CfxSnapshot(filename):
    ...
    noindex = np.genfromtxt(
        filename,
        skip_header=findStart(filename) + 2,
        delimiter=",",
        dtype=__readable_row,
    ).view(np.recarray)
    ...
    for el in __raw_row[1:]:
        ...
        index.__setattr__(
            key,
            U.convert(noindex.__getattribute__(key), fieldUnits[key], hlbUnits[key]),
        )
        continue
...
def parseHeader(filename, AllData=False):
    ...
    for field in header.split(","):
        ...
        try:
            ...
            niceField = field
            pass
        fieldUnits[niceField] = U.unit_parser.unit_parser.parse_unit(
            unit.replace("^", "**").replace(" ", "*"), suppress_unknown=False
        )
        continue
...
def findStart(filename, AllData=False):
    ...
    raise ValueError(
        "File '%s' does not appear to have a '[Data]' section." % self.filename
    )
...
class SteadyCfx(Cfx):
    ...
    def __getitem__(self, time):
        return Cfx.__getitem__(self, 0.0)
