...
class SnapCollection(collections.Mapping):
    ...
    def __getitem__(self, time):
        ...
        if i < n and self.times[i] - time <= self.tol:
            t = self.times[i]
        elif i > 0 and self.times[i - 1] - time <= self.tol:
            t = self.times[i - 1]
        else:
            raise KeyError(
                'No time close enough to "%s" tolerance %s' % (str(time), self.tol)
            )
...
class Heme(SnapCollection):
    ...
    def __init__(
        ...
        if timestep_s is None:
            raise ValueError(
                "Must supply keyword argument 'timestep_s', the length of a timestep in seconds."
            )
    ...
    def snapToTime(self, snap, delimiter="_"):
        ts = int(os.path.splitext(os.path.basename(snap))[0].split("_")[1])
...
class Diff(SnapCollection):
    ...
    def __getitem__(self, time):
        ...
        ans = np.recarray(shape=snap1.shape, dtype=fieldsD)
        ...
        for f in fieldsD:
            ...
            if name in ["position"]:
                assert np.allclose(
                    snap1.__getattribute__(name), snap2.__getattribute__(name)
                )
                ...
                ans.__setattr__(
                    name, snap2.__getattribute__(name) - snap1.__getattribute__(name)
                )
                pass
            continue
