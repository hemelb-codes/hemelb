...
class GeneratingLoader(ConfigLoader):
    ...
    def OnEndHeader(self):
        ...
        bounds = (
            np.concatenate((np.zeros(3, dtype=int), self.Domain.SiteCounts))
            .reshape((2, 3))
            .transpose()
        )
        # shift half a voxel down, scale and then translate
        bounds = (
            self.Domain.VoxelSize * (bounds - 0.5) + self.Domain.Origin[:, np.newaxis]
        )
        ...
        self.Locator.InitPointInsertion(
            self.Points, bounds.flatten(), self.TotalFluidSites
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("input", nargs=1, type=argparse.FileType(), help="the XML file")
    parser.add_argument(
        "output",
        nargs=argparse.OPTIONAL,
        help="file to write VTK data to, if omitted, use " 'input basename + ".vtu"',
        default=None,
    )
