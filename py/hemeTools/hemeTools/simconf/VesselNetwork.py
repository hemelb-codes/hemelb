...
class VesselSegment(object):
    ...
    def BuildTerms(self, inletPressure=None, flowRate=None, finalOutletPressure=0.0):
        unknowns = []
        ...
        if len(self.children):
            ...
            for ch in self.children:
                unknowns.extend(
                    ch.BuildTerms(
                        inletPressure=self.OutletPressure,
                        finalOutletPressure=finalOutletPressure,
                    )
                )
                continue
        else:
    ...
    def BuildSystem(self, prof):
        ...
        eq[self.InletPressure] = 1.0

        if isinstance(self.OutletPressure, Unknown):
            eq[self.OutletPressure] = -1.0
        else:
