...
namespace hemelb
{
  namespace tests
  {
    namespace net
    {
      LabelledRequest::LabelledRequest(void *pointer, int count, MPI_Datatype type, proc_t rank, const std::string &label) :
	SimpleRequest(pointer, count, type, rank), Label(label)
      {
      }

      bool LabelledRequest::EnvelopeIdentical(const SimpleRequest & other)
      {
	...
	return this_ok;
      ...
      bool LabelledRequest::PayloadIdentical(const SimpleRequest & other)
      {
	// reduction
	bool ok = true;
	...
	return ok;
      ...
      void LabelledRequest::Unpack(SimpleRequest & other)
      {
	for (int element = 0; element < Count; element++)
	  {
	    int size;
