...
namespace hemelb
{
  namespace tests
  {
	...
	site_t id = dom->GetGlobalNoncontiguousSiteIdFromGlobalCoords(exampleCoord);
	site_t blockSize = dom->GetBlockSize();
	REQUIRE(site_t( (1 * blockSize + 2) * blockSize + 3) == id);
	util::Vector3D<site_t> resultCoord;
	dom->GetGlobalCoordsFromGlobalNoncontiguousSiteId(id, resultCoord);
	...
	REQUIRE(dom->ProcProvidingSiteByGlobalNoncontiguousId(43) == 0);
      }
    }
  }
}
