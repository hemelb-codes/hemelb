...
void rtInitRayTracing(Mesh *mesh)
{
  ...
  {
    mesh->voxel[n].tri_ids = NULL;
    mesh->voxel[n].triangles = 0;
  }
  for (triangle_id n = 0; n < mesh->triangles; n++)
  {
    for (int l = 0; l < 3; l++)
    {
      aabb[l][0] = 1000000000;
    ...
    for (int m = 0; m < 3; m++)
      for (int l = 0; l < 3; l++)
      {
        voxel_id = (int) ( (mesh->triangle[n].v[m].pos[l] + mesh->half_dim[l])
            / mesh->voxel_size);
    ...
    for (i[0] = aabb[0][0]; i[0] <= aabb[0][1]; i[0]++)
      for (i[1] = aabb[1][0]; i[1] <= aabb[1][1]; i[1]++)
        for (i[2] = aabb[2][0]; i[2] <= aabb[2][1]; i[2]++)
          ++mesh->voxel[ (i[0] * mesh->voxels[1] + i[1]) * mesh->voxels[2]
              + i[2]].triangles;
  }
  for (int n = 0; n < mesh->voxels[3]; n++)
  {
    if (mesh->voxel[n].triangles == 0)
      continue;

    mesh->voxel[n].tri_ids = new triangle_id[mesh->voxel[n].triangles];
    mesh->voxel[n].triangles = 0;
  }
  for (triangle_id n = 0; n < mesh->triangles; n++)
  {
    ...
    for (i[0] = aabb[0][0]; i[0] <= aabb[0][1]; i[0]++)
      for (i[1] = aabb[1][0]; i[1] <= aabb[1][1]; i[1]++)
        for (i[2] = aabb[2][0]; i[2] <= aabb[2][1]; i[2]++)
        {
          ...
          mesh->voxel[voxel_id].tri_ids[mesh->voxel[voxel_id].triangles++]
              = n;
        }
  }
}
...
int rtVoxelVsRay(Voxel *voxel, Ray *ray, Hit *hit, Mesh *mesh)
{
  ...
  for (triangle_id m = 0; m < voxel->triangles; m++)
  {
    if (voxel->tri_ids[m] == hit->previous_triangle_id)
      continue;

    if (rtTriangleVsRay(&mesh->triangle[voxel->tri_ids[m]], ray, hit))
    {
      hit->tri_id = voxel->tri_ids[m];
    }
  }
  if (hit->tri_id != -1)
  {
...
int rtTraceRay(Ray *ray, Hit *hit, Mesh *mesh)
{
  ...
  for (l = 0; l < 3; l++)
    i[l] = max(0, min(mesh->voxels[l] - 1, (int) ( (ray->org[l] + ray->t_near
        * ray->dir[l] + mesh->half_dim[l]) / mesh->voxel_size)));

  if (ray->dir[0] < 0.0)
  {
    inc[0] = -1;
    t_max[0] = (i[0] * mesh->voxel_size - mesh->half_dim[0] - ray->org[0])
        / ray->dir[0];
  }
  else
  {
    inc[0] = 1;
    t_max[0] = ( (i[0] + 1) * mesh->voxel_size - mesh->half_dim[0]
        - ray->org[0]) / ray->dir[0];
  }
  if (ray->dir[1] < 0.0)
  {
    inc[1] = -1;
    t_max[1] = (i[1] * mesh->voxel_size - mesh->half_dim[1] - ray->org[1])
        / ray->dir[1];
  }
  else
  {
    inc[1] = 1;
    t_max[1] = ( (i[1] + 1) * mesh->voxel_size - mesh->half_dim[1]
        - ray->org[1]) / ray->dir[1];
  }
  if (ray->dir[2] < 0.0)
  {
    inc[2] = -1;
    t_max[2] = (i[2] * mesh->voxel_size - mesh->half_dim[2] - ray->org[2])
        / ray->dir[2];
  }
  else
  {
    inc[2] = 1;
    t_max[2] = ( (i[2] + 1) * mesh->voxel_size - mesh->half_dim[2]
        - ray->org[2]) / ray->dir[2];
  }
  for (l = 0; l < 3; l++)
  ...
  for (;;)
  {
    if (ray->t_near > ray->t_max)
      return !SUCCESS;

    ray->t_far = fmin(t_max[0], fmin(t_max[1], t_max[2]));

    voxel = &mesh->voxel[ (i[0] * mesh->voxels[1] + i[1]) * mesh->voxels[2]
        + i[2]];

    if (voxel->triangles > 0 && rtVoxelVsRay(voxel, ray, hit, mesh))
    {
      return SUCCESS;
    }
    if (t_max[0] < t_max[1])
...
int rtTracePrimaryRay(Hit *first_hit, Vis *vis)
{
  ...
  Rotate(screen.dim[0], 0.0, 0.0, viewpoint.sin_longitude,
         viewpoint.cos_longitude, viewpoint.sin_latitude,
         viewpoint.cos_latitude, &screen_dir_x[0], &screen_dir_x[1],
         &screen_dir_x[2]);

  Rotate(0.0, screen.dim[1], 0.0, viewpoint.sin_longitude,
         viewpoint.cos_longitude, viewpoint.sin_latitude,
         viewpoint.cos_latitude, &screen_dir_y[0], &screen_dir_y[1],
         &screen_dir_y[2]);

  for (l = 0; l < 3; l++)
  {
    screen_vtx[l] = (-viewpoint.dist / vis->viewpoint_radius)
        * viewpoint.pos[l] - screen_dir_x[l] - screen_dir_y[l];
  ...
  for (l = 0; l < 3; l++)
    ray.dir[l] = screen_vtx[l] + vis->mouse.x[0] * screen_dir_x[l]
        + vis->mouse.x[1] * screen_dir_y[l];
  ...
  if (ray.t_near > ray.t_far || rtTraceRay(&ray, first_hit, &vis->mesh)
      != SUCCESS)
  {
    return !SUCCESS;
  }
...
int rtTraceSecondaryRay(Hit *first_hit, Hit *second_hit, Vis *vis)
{
  ...
  for (l = 0; l < 3; l++)
    ray.dir[l] = vis->mesh.triangle[first_hit->tri_id].nor[l];
...
void rtEndRayTracing(Mesh *mesh)
{
  for (int n = 0; n < mesh->voxels[3]; n++)
  {
    if (mesh->voxel[n].tri_ids != NULL)
    {
      free(mesh->voxel[n].tri_ids);
      mesh->voxel[n].tri_ids = NULL;
    }
  }
  free(mesh->voxel);
