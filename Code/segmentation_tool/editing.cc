...
{
  for (int l = 0; l < 3; l++)
    c[l] = (1.0/3.0) * (t_p->v[0].pos[l]+t_p->v[1].pos[l]+t_p->v[2].pos[l]);
}
...
void editTriangleNormal (BoundaryTriangle *t_p, double nor[3])
{
  ...
  temp = 1.0 / fmax(EPSILON, sqrt(nor[0]*nor[0] + nor[1]*nor[1] + nor[2]*nor[2]));
...
void editCalculateTriangleData (BoundaryTriangle *t_p)
{
  ...
  latitude  = atan2f(t_p->nor[1], sqrt(t_p->nor[0]*t_p->nor[0] + t_p->nor[2]*t_p->nor[2]));
...
void editChangeTrianglePars (int b_id, int t_id, double dp_avg, double dp_amp, double dp_phs, Vis *vis)
{
  ...
  t_p->pressure_amp = fmin(t_p->pressure_avg, fmax(0.0, 0.1 * (int)(10.0 * t_p->pressure_amp)));
	  ...
	  for (int l = 0; l < 3; l++)
	    {
	      ...
	      voxel[0] = (int)((vis->screen_voxels/(2.0*screen.dim[0])) * (x[0]+screen.dim[0]));
	      voxel[1] = (int)((vis->screen_voxels/(2.0*screen.dim[1])) * (x[1]+screen.dim[1]));
...
void editMoveTriangleVertexWithMouse (int x0[3], Vis *vis)
{
  ...
  x1[0] = screen.dim[0] * (-1.0 + (x0[0]<<1) / (double)vis->viewport_pixels[0]);
  x1[1] = screen.dim[1] * (-1.0 + (x0[1]<<1) / (double)vis->viewport_pixels[1]);
  x1[2] = x0[2];
...
void editRotateTriangleWithMouse (double dx0, double dy0, int b_id, int t_id, Vis *vis)
{
  ...
  for (i = 0; i < 3; i++)
	...
	dx1[l] = t_p->v[i].pos[l] - c[l];
      ...
      AntiRotate (dx1[0], dx1[1], dx1[2],
		  sin(longitude[0]), cos(longitude[0]),
		  sin(latitude[0]), cos(latitude[0]),
		  &dx2[0], &dx2[1], &dx2[2]);
      
      Rotate (dx2[0], dx2[1], dx2[2],
	      sin(longitude[1]), cos(longitude[1]),
	      sin(latitude[1]), cos(latitude[1]),
	      &dx3[0], &dx3[1], &dx3[2]);
...
void editMotionFunction (int x, int y, Vis *vis)
{
  ...
  if (vis->mode == 0)
	...
	{
	  vis->mouse.dy = 10 * mouse_dy;
	}
      else if (vis->menu.option == CHANGE_SLICE)
	{
	...
	{
	  editRotateViewpointWithMouse (-(double)mouse_dx / vis->viewport_pixels[0],
					-(double)mouse_dy / vis->viewport_pixels[1], vis);
	}
#ifndef MESH
      else if (vis->menu.option == CHANGE_THRESHOLD)
	{
	  vis->mouse.dy = 10 * mouse_dy;
	}
#endif
	       ...
	       ( ((unsigned int)vis->mouse.b_id) == INLET_BOUNDARY || ((unsigned int) vis->mouse.b_id) == OUTLET_BOUNDARY))
	{
	  ...
	  if (vis->menu.option & CHANGE_MEAN_PRESSURE)
	    {
	      editChangeTrianglePars (vis->mouse.b_id, vis->mouse.t_id, dy, 0.0, 0.0, vis);
	    }
	  else if (vis->menu.option & CHANGE_PRESSURE_AMPLITUDE)
	    {
	      editChangeTrianglePars (vis->mouse.b_id, vis->mouse.t_id, 0.0, dy, 0.0, vis);
	    }
	  else
	    {
	      editChangeTrianglePars (vis->mouse.b_id, vis->mouse.t_id, 0.0, 0.0, dy, vis);
	    }
	}
    }
  if (vis->mode == 2)
