...
void ioReadConfig(Vis *vis)
{
  ...
  while (strcmp(key_word, "endsolid"))
  {
    ...
    if (vis->mesh.triangles == vis->mesh.triangles_max)
    {
      ...
      vis->mesh.triangle
          = (MeshTriangle *) realloc(vis->mesh.triangle, sizeof(MeshTriangle)
              * vis->mesh.triangles_max);
    }
    ...
    for (int l = 0; l < 3; l++)
      vis->mesh.triangle[vis->mesh.triangles].nor[l]
          /= sqrt(ScalarProd(vis->mesh.triangle[vis->mesh.triangles].nor,
                             vis->mesh.triangle[vis->mesh.triangles].nor));

    // Scan to loop
    while (strcmp(key_word, "loop"))
      input_file >> key_word;

    for (int m = 0; m < 3; m++) // Must be a triangle
    ...
    for (int m = 0; m < 3; m++) // vertices
      for (int l = 0; l < 3; l++) // dimensions

      {
        triangle_ctr_x[l]
            += vis->mesh.triangle[vis->mesh.triangles].v[m].pos[l];
      }
    for (int l = 0; l < 3; l++)
...

    {
...

      {
        ...
        vis->mesh.triangle[vis->mesh.triangles].v[m].pos[l] = triangle_ctr_x[l]
            + 1.01 * (vis->mesh.triangle[vis->mesh.triangles].v[m].pos[l]
                - triangle_ctr_x[l]);
      }
    }
  ...
  for (triangle_id n = 0; n < vis->mesh.triangles; n++) // for each tri

  {
    // Translate such that vis->mesh.centre is the origin
    for (int m = 0; m < 3; m++) // vertex
      for (int l = 0; l < 3; l++) // dimen
        vis->mesh.triangle[n].v[m].pos[l] -= vis->mesh.centre[l];

    // Add perimeter of tri to vis->mesh.voxel_size
    for (int l = 0; l < 3; l++)
      dx[l] = vis->mesh.triangle[n].v[1].pos[l]
          - vis->mesh.triangle[n].v[0].pos[l];

    vis->mesh.voxel_size
        += sqrtf(dx[0] * dx[0] + dx[1] * dx[1] + dx[2] * dx[2]);

    for (int l = 0; l < 3; l++)
      dx[l] = vis->mesh.triangle[n].v[2].pos[l]
          - vis->mesh.triangle[n].v[0].pos[l];
  ...
  for (int l = 0; l < 3; l++)
    vis->mesh.voxels[l] = (int) ceil(1.5 * (MaxXValue[l] - min_x[l])
        / vis->mesh.voxel_size);

  // This is the total number of voxels
  vis->mesh.voxels[3] = vis->mesh.voxels[0] * vis->mesh.voxels[1]
      * vis->mesh.voxels[2];
  ...
  for (block_id n = 0; n < vis->tot_blocks; n++)
    vis->block[n].site = NULL;

  vis->stack_sites_max = SITES_PER_BLOCK * 1000000;
...
void ioReadCheckpoint(Vis *vis)
{
  ...
  {
    xdr_long(&xdr_config, &vis->boundary[n].triangles);
...
void ioWriteConfig(Vis *vis)
{
  ...
  xdr_long(&xdr_config, &vis->blocks[0]);
  xdr_long(&xdr_config, &vis->blocks[1]);
  xdr_long(&xdr_config, &vis->blocks[2]);
  ...
  for (block_id i = 0; i < vis->blocks[0] * BLOCK_SIZE; i += BLOCK_SIZE)
  {
    for (block_id j = 0; j < vis->blocks[1] * BLOCK_SIZE; j += BLOCK_SIZE)
    {
      for (block_id k = 0; k < vis->blocks[2] * BLOCK_SIZE; k += BLOCK_SIZE)
      {
        n++;
          ...
          for (site[1] = j; site[1] < j + BLOCK_SIZE; site[1]++)
          {
            for (site[2] = k; site[2] < k + BLOCK_SIZE; site[2]++)
            {
              ...
              if (*site_type == SOLID_TYPE)
              {
                ...
                xdr_u_long(&xdr_config, site_type);
              ...
              {
                // Site is complicated!
                segCalculateBoundarySiteData(*site_type, site, boundary_nor,
                                             &boundary_dist, wall_nor,
                                             &wall_dist, cut_dist, vis);
                ...
                if ( (*site_type & SITE_TYPE_MASK) == FLUID_TYPE || (*site_type
                    & PRESSURE_EDGE_MASK))
                {
                  for (int l = 0; l < 3; l++)
...
void ioWritePars(Vis *vis)
{
  ...
  for (triangle_id n = 0; n < vis->boundary[INLET_BOUNDARY].triangles; n++)
  {
    ...
    fprintf(pars, "%e %e %e\n", t_p->pressure_avg, t_p->pressure_amp,
            t_p->pressure_phs);
  }

  fprintf(pars, "%lu\n", vis->boundary[OUTLET_BOUNDARY].triangles);

  for (triangle_id n = 0; n < vis->boundary[OUTLET_BOUNDARY].triangles; n++)
  {
...
void ioWriteCoords(Vis *vis)
{
  ...
  fprintf(coords, "voxels =  %d, %d, %d\n", vis->mesh.voxels[0],
          vis->mesh.voxels[1], vis->mesh.voxels[2]);
  fprintf(coords, "dim =  %e, %e, %e\n", vis->mesh.dim[0], vis->mesh.dim[1],
          vis->mesh.dim[2]);
  fprintf(coords, "half_dim = %e, %e, %e\n", vis->mesh.half_dim[0],
          vis->mesh.half_dim[1], vis->mesh.half_dim[2]);
  fprintf(coords, "mesh_centre = %e, %e, %e\n", vis->mesh.centre[0],
          vis->mesh.centre[1], vis->mesh.centre[2]);
  fprintf(coords, "seed_pos = %e, %e, %e\n", vis->seed_pos[0],
          vis->seed_pos[1], vis->seed_pos[2]);
  fprintf(coords, "seed_site = %li, %li, %li\n", vis->seed_site[0],
          vis->seed_site[1], vis->seed_site[2]);
