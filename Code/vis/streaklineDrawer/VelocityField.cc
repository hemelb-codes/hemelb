  ...
  namespace vis
  {
    namespace streaklinedrawer
    {
                                             ...
                                             StreaklineDrawer* iStreaklineDrawer)
      {
        ...
        do
        {
          geometry::LatticeData::BlockData* lBlock = lBlockTraverser.GetCurrentBlockData();
          ...
          do
          {
            ...
            iStreaklineDrawer->mParticleSeeds.push_back(Particle(static_cast<float>(lBlockTraverser.GetX()
                                                                     * lBlockTraverser.GetBlockSize()
                                                                     + lSiteTraverser.GetX()), static_cast<float>(lBlockTraverser.GetY()*lBlockTraverser.GetBlockSize() +
                                                                     lSiteTraverser.GetY()),
      ...
      VelocitySiteData* VelocityField::velSiteDataPointer(const geometry::LatticeData& iLatDat,
                                                          ...
                                                          site_t site_k)
      {
        if (site_i >= iLatDat.GetXSiteCount() || site_j >= iLatDat.GetYSiteCount()
            || site_k >= iLatDat.GetZSiteCount())
        {
          return NULL;
        }
        ...
        site_t site_id = ( ( (ii << iLatDat.GetLog2BlockSize()) + jj) << iLatDat.GetLog2BlockSize())
            + kk;
                                                  ...
                                                  StreaklineDrawer* iStreaklineDrawer)
      {
        ...
        {
          mVelocityField[block_id] = std::vector < VelocitySiteData
              > (iLatDat.GetSitesPerBlockVolumeUnit());
        }
