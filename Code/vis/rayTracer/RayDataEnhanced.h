  ...
  namespace vis
  {
    namespace raytracer
    {
      ...
      class RayDataEnhanced : public RayData<RayDataEnhanced<depthCuing> >
      {
        public:
          RayDataEnhanced() :
              mVelocitySum(0.0F), mStressSum(0.0F), mSurfaceNormalLightness(1.0F)
          {
          }
                                   ...
                                   const DomainStats& iDomainStats) const
          {
            float lVelocityHue = mVelocityHueRange * GetAverageVelocity()
                * static_cast<float>(iDomainStats.velocity_threshold_max_inv) + mVelocityHueMin;
          ...
          void DoMergeIn(const RayDataEnhanced& iOtherRayData, const VisSettings& iVisSettings)
          {
          ...
          float GetLightnessValue(const float iNormalisedDistance) const
          {
            ...
            else if (depthCuing == DepthCuing::DARKNESS)
            {
              ...
              float lLightnessValue = 0.8F * (1.0F - iNormalisedDistance)
                  + (GetSurfaceNormalLightness() - 1.0F) * mSurfaceNormalLightnessRange;
