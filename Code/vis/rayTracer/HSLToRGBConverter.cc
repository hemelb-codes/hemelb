...
namespace hemelb
{
  namespace vis
  {
	...
	static const uint32_t OtherScalar = 
	  std::numeric_limits<uint16_t>::max();

	uint32_t lHue = static_cast<uint32_t>(iHue*static_cast<float>(DegreesScalar));

	uint32_t lSaturation = static_cast<uint32_t>
	  (iSaturation*static_cast<float>(OtherScalar));
	
	uint32_t lLightness = static_cast<uint32_t>
	  (iLightness*static_cast<float>(OtherScalar));
	
	int32_t lTemp = 2*static_cast<int32_t>(lLightness) - 
	   static_cast<int32_t>(OtherScalar);
	
	uint32_t lChroma = (
	  (OtherScalar-abs(lTemp))*lSaturation
	  )/OtherScalar;

	uint32_t lHuePrime = lHue/60;

	lTemp = static_cast<int>(lHuePrime%(2*DegreesScalar)) -
	  static_cast<int>(DegreesScalar);

	uint32_t lIntermediate = (
	  lChroma*
	  (DegreesScalar-
	   abs(lTemp))
	  )/DegreesScalar;
	...
	oRGBColour[0] = static_cast<unsigned char>((lRed + lMatcher)/CharScalar);
	oRGBColour[1] = static_cast<unsigned char>((lGreen + lMatcher)/CharScalar);
	oRGBColour[2] = static_cast<unsigned char>((lBlue + lMatcher)/CharScalar);
      }
    }
  }
}
