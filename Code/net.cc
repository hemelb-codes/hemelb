...
void Net::InitialiseSendReceive(hemelb::lb::LocalLatticeData &bLocalLatDat)
{
      ...
      mNetworkTopology->NeighbouringProcs.begin(); it != mNetworkTopology->NeighbouringProcs.end(); it++)
  {
    ...
    RequestSend<double> (&bLocalLatDat.FNew[ (*it)->FirstSharedF], (*it)->SharedFCount, (*it)->Rank);
  }
}
...
void Net::ReceiveFromNeighbouringProcessors(hemelb::lb::LocalLatticeData &bLocalLatDat)
{
  ...
  {
    MPI_Irecv(it->second->mReceivePointerList.front(), 1, it->second->ReceiveType, it->first, 10,
...
void Net::AddToList(double* iNew,
                    int iLength,
                    std::vector<void*> &iPointerList,
                    std::vector<int> &iLengthList,
                    std::vector<MPI_Datatype> &iTypeList)
