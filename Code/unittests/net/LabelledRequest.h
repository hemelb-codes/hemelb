    ...
    namespace net
    {
      class LabelledRequest : public hemelb::net::SimpleRequest
      {
        public:
          ...
          LabelledRequest(void *pointer, int count, MPI_Datatype type, proc_t rank,
                          const std::string &label) :
          ...
          virtual bool EnvelopeIdentical(const SimpleRequest & other)
          {
            ...
            if (!this_ok)
            {
              std::cerr << "Envelope different: " << Label << " R: " << Rank << " C: " << Count
                  << " T: " << " : " << " R" << other.Rank << " C " << other.Count << " T "
                  << std::endl;
          ...
          virtual bool PayloadIdentical(const SimpleRequest & other)
          {
            ...
            for (int element = 0; element < Count; element++)
            {
              ...
              if (!this_ok)
              {
                ...
                std::cerr << "Unexpected data in request: " << Label << " R " << Rank << " C "
                    << Count << " : " << std::endl;
                for (int i = 0; i < size; i++)
                {
                  ...
                  // But we cannot currently see a better solution to avoid compiler warnings from void* arithmetic.
                  std::cerr << i << " : "
                      << static_cast<int>(* (static_cast<unsigned char*>(Pointer) + size * element
                          + i)) << " "
                      << static_cast<int>(* (static_cast<unsigned char*>(other.Pointer)
                          + size * element + i)) << std::endl;
                }
                std::cerr << std::endl;
              }
