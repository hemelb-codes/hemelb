      ...
      class ReporterTests : public CppUnit::TestFixture
      {
        ...
        public:
          ...
          void back_has_substring(const std::string &expectation){
            ...
            CPPUNIT_ASSERT_MESSAGE(expectation+" : "+actual,std::string::npos!=actual.find(expectation));
          ...
          void TestMainReport(){
            ...
            reporter->Cycle();reporter->Cycle();
            reporter->Snapshot();reporter->Snapshot();reporter->Snapshot();reporter->Snapshot();
            reporter->Image();reporter->Image();reporter->Image();
            for (unsigned int step=0;step<1000;step++){reporter->TimeStep();}
          ...
          void table_expectation(unsigned int row){
            ...
            double normn=1.0;
            if (row==Timers::snapshot){normn=4.0;}
            if (row==Timers::visualisation){normn=3.0;}
            if (row==Timers::lb || row==Timers::mpiSend || row==Timers::mpiWait ){normn=2.0;}
            expectation<<timerNames[row]<<"\t\t"<<row*10.0<<"\t"<<row*15.0/normn<<"\t"<<row*10.0/normn<<"\t"<<row*5.0/normn<<std::flush;
            back_has_substring(expectation.str());
          }
