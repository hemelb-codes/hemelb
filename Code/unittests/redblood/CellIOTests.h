      ...
      class CellIOTests : public helpers::FolderTestFixture
      {
        ...
        public:
          ...
          void testReadCellWithDefaults()
          {
            ...
            doc.FirstChildElement("parent")->FirstChildElement("cell")->RemoveChild(
                doc.FirstChildElement("parent")->FirstChildElement("cell")
                    ->FirstChildElement("moduli"));

            auto const cellbase = read_cell(doc.FirstChildElement("parent"), *converter);
            auto const cell = std::static_pointer_cast<Cell>(cellbase);
            ...
            CPPUNIT_ASSERT_DOUBLES_EQUAL(
                converter->ConvertToLatticeUnits("m", scale), cell->GetScale(), 1e-12);
            CPPUNIT_ASSERT_DOUBLES_EQUAL(1e0, cell->moduli.volume, 1e-12);
            CPPUNIT_ASSERT_DOUBLES_EQUAL(1e0, cell->moduli.surface, 1e-12);
            CPPUNIT_ASSERT_DOUBLES_EQUAL(
                converter->ConvertToLatticeUnits("Nm", 2e-19), cell->moduli.bending, 1e-12);
            CPPUNIT_ASSERT_DOUBLES_EQUAL(
                converter->ConvertToLatticeUnits("N/m", 5e-1), cell->moduli.dilation, 1e-12);
            CPPUNIT_ASSERT_DOUBLES_EQUAL(
                converter->ConvertToLatticeUnits("N/m", 5e-6), cell->moduli.strain, 1e-12);
          ...
          void testReadCellModuli()
          {
            ...
            CPPUNIT_ASSERT_DOUBLES_EQUAL(
                converter->ConvertToLatticeUnits("Nm", 2e-18), moduli.bending, 1e-12);
            CPPUNIT_ASSERT_DOUBLES_EQUAL(
