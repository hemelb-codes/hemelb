      ...
      class IncompressibilityCheckerTests : public helpers::FourCubeBasedTestFixture
      {
        ...
        public:
          ...
          void TestIncompressibilityChecker()
          {
            ...
            net::Net net;
            hemelb::lb::IncompressibilityChecker<net::BroadcastMock, lb::lattices::D3Q15> incompChecker(latDat,
                                                                                                        &net,
                                                                                                        simState,
                                                                                                        timings,
            ...
            distribn_t largestDefaultDensity = (numDirections * (numDirections + 1) / 20) + ( (numSites - 1)
                * numDirections / 100); // = sum_{j=1}^{numDirections} j/10 + (numSites-1)/100 = 21.45 with current configuration of FourCubeLatticeData
            ...
            CPPUNIT_ASSERT_DOUBLES_EQUAL( (largestDefaultDensity - smallestDefaultDensity)
                / hemelb::lb::REFERENCE_DENSITY, incompChecker.GetMaxRelativeDensityDifference(), eps);
