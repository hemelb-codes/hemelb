      ...
      public:
        ...
        void testCalculatDensityAndMomentum()
        {
          ...
          for (size_t i(0); i < lb::lattices::D3Q15::NUMVECTORS; ++i)
          {
            ...
            expected_momentum += PhysicalVelocity(D3Q15::CX[i], D3Q15::CY[i], D3Q15::CZ[i]) * f[i];
          }
        ...
        void testCalculateDensityMomentumFEq()
        {
          ...
          distribn_t noforce_feq[D3Q15::NUMVECTORS], force_feq[D3Q15::NUMVECTORS],
              feq[D3Q15::NUMVECTORS];
        ...
        void testForceDistributionColinear()
        {
          ...
          for (size_t i(0); i < D3Q15::NUMVECTORS; ++i)
          {
            ...
            distribn_t const expected = (1. - 0.5 / 0.25) * D3Q15::EQMWEIGHTS[i]
                * (ei_norm * ei_norm / 9.0);
        ...
        void testForceDistributionZeroVelocity()
        {
          ...
          for (size_t i(0); i < D3Q15::NUMVECTORS; ++i)
          {
            ...
            distribn_t ei_norm(ei.Dot(ei));
            distribn_t const expected = (1. - 0.5 / 0.25) * D3Q15::EQMWEIGHTS[i] * (ei_norm / 3.0);
            CPPUNIT_ASSERT_DOUBLES_EQUAL(Fi[i], expected, 1e-8);
          }
        }
        ...
        void testSimpleCollideAndStream()
        {
          ...
          FPostCollision(site.GetFOld<D3Q15>(), LatticeForceVector(0, 0, 0), withoutForce);
          ...
          for (size_t i(0); i < D3Q15::NUMVECTORS; ++i)
          {
            ...
            CPPUNIT_ASSERT_DOUBLES_EQUAL(withForce[i],
        ...
        void testSimpleBounceBack()
        {
          ...
          distribn_t const * const actual = helpers::GetFNew<D3Q15>(latDat, position);
          bool paranoia(false);
          for (size_t i(0); i < D3Q15::NUMVECTORS; ++i)
          {
            if (not site.HasWall(i))
              continue;
            paranoia = true;
            CPPUNIT_ASSERT_DOUBLES_EQUAL(withForce[i], actual[D3Q15::INVERSEDIRECTIONS[i]], 1e-8);
            // And that forces from streaming site were used
            CPPUNIT_ASSERT(not helpers::is_zero(withForce[i] - withoutForce[i]));
          }
