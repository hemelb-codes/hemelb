...
namespace hemelb
{
  namespace redblood
  {
    ...
    {
      return firstCellNode != lastCellNode
        and box_iterator
    ...
    WallCellPairIterator::WallCellPairIterator(
       ...
       : cellNodes(cellNodes), wallNodes(wallNodes), firstCellNode(firstCellNode),
         lastCellNode(lastCellNode), firstWallNode(firstWallNode), box_iterator(0), cutoff(cutoff)
     {
       if(firstWallNode == wallNodes.end())
       {
         return;
       ...
       try
       {
         ...
         if(firstCellNode == lastCellNode)
         {
           std::tie(this->firstCellNode, this->lastCellNode)
             = cellNodes(firstWallNode->second.node + *box_iterator);
         }
         if(not isValid())
         {
...
namespace std
{
  ...
  {
    using namespace hemelb::redblood;
    return {get<0>(args), get<1>(args), get<2>(args), WallCellPairIterator::Begin()};
  }
  ...
  {
    using namespace hemelb::redblood;
    return {get<0>(args), get<1>(args), get<2>(args), WallCellPairIterator::End()};
  }
} // std
