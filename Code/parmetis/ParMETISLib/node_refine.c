         ...
         char *title)
{
  ...
  if (ctrl->mype == 0) {
    printf("%s: %" IDXTYPE_FORMAT ", ", title, gpwgts[2*nparts-1]);

    for (i=0; i<nparts; i+=2) 
      printf(" [%5" IDXTYPE_FORMAT " %5" IDXTYPE_FORMAT " %5" IDXTYPE_FORMAT " %5" IDXTYPE_FORMAT "]", gpwgts[i], gpwgts[i+1], gpwgts[nparts+i], badmaxpwgt[i]);
    printf("\n");
  }
