    ...
    if (mype == penum) {
      ...
      for (i=0; i<graph->nvtxs; i++) {
        for (h=0; h<graph->ncon; h++)
          fprintf(fpin, "%" IDXTYPE_FORMAT " ", graph->vwgt[i*graph->ncon+h]);

        for (j=graph->xadj[i]; j<graph->xadj[i+1]; j++) {
          fprintf(fpin, "%" IDXTYPE_FORMAT " ", graph->adjncy[j]+1);
          fprintf(fpin, "%" IDXTYPE_FORMAT " ", graph->adjwgt[j]);
        }
      fprintf(fpin, "\n");
      }
      fclose(fpin);
...
void ReadTestGraph(GraphType *graph, char *filename, MPI_Comm comm)
{
  ...
  if (mype == 0) {
    ...
    printf("Nvtxs: %d, Nedges: %" IDXTYPE_FORMAT "\n", nvtxs, gxadj[nvtxs]);
...
void WriteOVector(char *gname, idxtype *vtxdist, idxtype *order, MPI_Comm comm)
{
  ...
  if (mype == 0) {
    ...
    for (i=0; i<vtxdist[npes]; i++) {
      if (gorder[i] != 1)
        printf("Global ordering problems with index: %d [%" IDXTYPE_FORMAT "]\n", i, gorder[i]);
    }
