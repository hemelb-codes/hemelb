...
void Random_KWayEdgeRefineMConn(CtrlType *ctrl, GraphType *graph, int nparts, float *tpwgts, float ubfactor, int npasses, int ffactor)
{
  ...
  IFSET(ctrl->dbglvl, DBG_REFINE,
     printf("Partitions: [%6" IDXTYPE_FORMAT " %6" IDXTYPE_FORMAT "]-[%6" IDXTYPE_FORMAT " %6" IDXTYPE_FORMAT "], Balance: %5.3f, Nv-Nb[%6d %6d]. Cut: %6d\n",
  ...
  for (pass=0; pass<npasses; pass++) {
    ...
    for (nmoves=iii=0; iii<graph->nbnd; iii++) {
      ...
      if (myrinfo->ed >= myrinfo->id) { /* Total ED is too high */
        ...
        *======================================================================*/
        ...
        IFSET(ctrl->dbglvl, DBG_MOVEINFO, printf("\t\tMoving %6d to %3d. Gain: %4" IDXTYPE_FORMAT ". Cut: %6d\n", i, to, myedegrees[k].ed-myrinfo->id, graph->mincut));
          ...
          if (me != from && me != to) {
            ...
            if (pmat[me*nparts+to] == 0) {
              ...
              if (ndoms[me] > maxndoms) {
                IFSET(ctrl->dbglvl, DBG_REFINE, printf("You just increased the maxndoms: %" IDXTYPE_FORMAT " %d\n", ndoms[me], maxndoms));
                maxndoms = ndoms[me];
              }
            }
    ...
    IFSET(ctrl->dbglvl, DBG_REFINE,
       printf("\t[%6" IDXTYPE_FORMAT " %6" IDXTYPE_FORMAT "], Balance: %5.3f, Nb: %6d. Nmoves: %5d, Cut: %5d, Vol: %5d, %d\n",
...
void Greedy_KWayEdgeBalanceMConn(CtrlType *ctrl, GraphType *graph, int nparts, float *tpwgts, float ubfactor, int npasses)
{
  ...
  IFSET(ctrl->dbglvl, DBG_REFINE,
     printf("Partitions: [%6" IDXTYPE_FORMAT " %6" IDXTYPE_FORMAT "]-[%6" IDXTYPE_FORMAT " %6" IDXTYPE_FORMAT "], Balance: %5.3f, Nv-Nb[%6d %6d]. Cut: %6d [B]\n",
             pwgts[idxamin(nparts, pwgts)], pwgts[idxamax(nparts, pwgts)], minwgt[0], maxwgt[0], 
             1.0*nparts*pwgts[idxamax(nparts, pwgts)]/tvwgt, graph->nvtxs, graph->nbnd,
             graph->mincut));
  ...
  for (pass=0; pass<npasses; pass++) {
    ...
    IFSET(ctrl->dbglvl, DBG_REFINE,
       printf("\t[%6" IDXTYPE_FORMAT " %6" IDXTYPE_FORMAT "], Balance: %5.3f, Nb: %6d. Nmoves: %5d, Cut: %6d, %d\n",
               pwgts[idxamin(nparts, pwgts)], pwgts[idxamax(nparts, pwgts)],
