                          ...
                          float ubfactor, int npasses, int ffactor)
{
  ...
  IFSET(ctrl->dbglvl, DBG_REFINE,
     printf("VolPart: [%5" IDXTYPE_FORMAT " %5" IDXTYPE_FORMAT "]-[%5" IDXTYPE_FORMAT " %5" IDXTYPE_FORMAT "], Balance: %3.2f, Nv-Nb[%5d %5d]. Cut: %5d, Vol: %5d\n",
  ...
  for (pass=0; pass<npasses; pass++) {
    ...
    for (nmoves=iii=0; iii<graph->nbnd; iii++) {
      ...
      if (myrinfo->gv >= 0) { /* Total volume gain is too high */
        ...
        *======================================================================*/
        ...
        IFSET(ctrl->dbglvl, DBG_MOVEINFO, printf("\t\tMoving %6d from %3d to %3d. Gain: [%4" IDXTYPE_FORMAT " %4" IDXTYPE_FORMAT "]. Cut: %6d, Vol: %6d\n",
    ...
    IFSET(ctrl->dbglvl, DBG_REFINE,
       printf("\t[%6" IDXTYPE_FORMAT " %6" IDXTYPE_FORMAT "], Balance: %5.3f, Nb: %6d. Nmoves: %5d, Cut: %6d, Vol: %6d\n",
                           ...
                           float ubfactor, int npasses)
{
  ...
  IFSET(ctrl->dbglvl, DBG_REFINE,
     printf("VolPart: [%5" IDXTYPE_FORMAT " %5" IDXTYPE_FORMAT "]-[%5" IDXTYPE_FORMAT " %5" IDXTYPE_FORMAT "], Balance: %3.2f, Nv-Nb[%5d %5d]. Cut: %5d, Vol: %5d [B]\n",
             pwgts[idxamin(nparts, pwgts)], pwgts[idxamax(nparts, pwgts)], minwgt[0], maxwgt[0], 
             1.0*nparts*pwgts[idxamax(nparts, pwgts)]/tvwgt, graph->nvtxs, graph->nbnd,
             graph->mincut, graph->minvol));
      ...
      for (kk=0; kk<tmprinfo.ndegrees; kk++) {
        if (tmpdegrees[kk].pid == pid) {
          if (tmpdegrees[kk].gv != myedegrees[k].gv)
            printf("[%d %d %" IDXTYPE_FORMAT " %" IDXTYPE_FORMAT "]\n", i, pid, myedegrees[k].gv, tmpdegrees[kk].gv);
          break;
        }
      }
    }
