...
#ifndef HEMELB_LB_STREAMERS_ADVECTIONDIFFUSIONOUTFLOWSTREAMERTYPEFACTORY_H
#define HEMELB_LB_STREAMERS_ADVECTIONDIFFUSIONOUTFLOWSTREAMERTYPEFACTORY_H
...
namespace hemelb
{
  namespace lb
  {
    namespace streamers
    {
      ...
      class AdvectionDiffusionOutflowIoletStreamerTypeFactory : public AdvectionDiffusionBaseStreamer<AdvectionDiffusionOutflowIoletStreamerTypeFactory<CollisionImpl, IoletLinkImpl> >
      {
        public:
        ...
        public:
          AdvectionDiffusionOutflowIoletStreamerTypeFactory(kernels::InitParams& initParams) :
                                         ...
                                         lb::MacroscopicPropertyCache& coupledPropertyCache)
          {
            for (site_t siteIndex = firstIndex; siteIndex < (firstIndex + siteCount); siteIndex++)
            {
              ...
              for (Direction ii = 0; ii < LatticeType::NUMVECTORS; ii++)
              {
                if (site.HasIolet(ii))
                {
                  ioletLinkDelegate.StreamLink(lbmParams, latDat, site, hydroVars, coupledPropertyCache.velocityCache.Get(site.GetIndex()).x,
                                                                                   coupledPropertyCache.velocityCache.Get(site.GetIndex()).y,
                                                                                   coupledPropertyCache.velocityCache.Get(site.GetIndex()).z, ii);
              ...
              AdvectionDiffusionBaseStreamer<AdvectionDiffusionOutflowIoletStreamerTypeFactory>::template UpdateMinsAndMaxes<tDoRayTracing>(site,
                                                                                                                                            hydroVars,
                                                                                                                                            lbmParams,
                                                                                                                                            propertyCache);
            }
      ...
      class AdvectionDiffusionWallOutflowIoletStreamerTypeFactory : public AdvectionDiffusionBaseStreamer<AdvectionDiffusionWallOutflowIoletStreamerTypeFactory<CollisionImpl,
          WallLinkImpl, IoletLinkImpl, StentLinkImpl> >
      {
        ...
        public:
          AdvectionDiffusionWallOutflowIoletStreamerTypeFactory(kernels::InitParams& initParams) :
                                         ...
                                         lb::MacroscopicPropertyCache& coupledPropertyCache)
          {
            for (site_t siteIndex = firstIndex; siteIndex < (firstIndex + siteCount); siteIndex++)
            {
              ...
              //TODO: Necessary to specify sub-class?
              AdvectionDiffusionBaseStreamer<AdvectionDiffusionWallOutflowIoletStreamerTypeFactory>::template UpdateMinsAndMaxes<tDoRayTracing>(site,
                                                                                                                                                hydroVars,
                                                                                                                                                lbmParams,
                                                                                                                                                propertyCache);
            }
...
#endif // HEMELB_LB_STREAMERS_ADVECTIONDIFFUSIONOUTFLOWSTREAMERTYPEFACTORY_H
