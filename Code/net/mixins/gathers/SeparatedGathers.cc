...
namespace hemelb
{
  namespace net
  {
    void SeparatedGathers::WaitGathers()
    {
       ...
       */
      for (std::map<proc_t, GatherProcComms>::iterator send_it = gatherSendProcessorComms.begin(); send_it
          != gatherSendProcessorComms.end(); ++send_it)
      {
        /*
        ...
        if (send_it->first == communicator.Rank())
        {
           ...
           */
          int gather_index = 0;
          for (GatherProcComms::iterator receive_it = gatherReceiveProcessorComms.begin(); receive_it
              != gatherReceiveProcessorComms.end(); ++receive_it)
          {
         ...
         */
        {
          for (GatherProcComms::iterator req = send_it->second.begin(); req
              != send_it->second.end(); req++)
          {
            HEMELB_MPI_CALL(
                MPI_Gather,
    ...
    void SeparatedGathers::WaitGatherVs()
    {
      ...
      for (std::map<proc_t, ProcComms>::iterator send_it = gatherVSendProcessorComms.begin(); send_it
          != gatherVSendProcessorComms.end(); ++send_it)
      {
        ...
        if (send_it->first == communicator.Rank())
        {
          ...
          for (GatherVReceiveProcComms::iterator receive_it = gatherVReceiveProcessorComms.begin(); receive_it
              != gatherVReceiveProcessorComms.end(); ++receive_it)
          {
            /***
