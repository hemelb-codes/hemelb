	...
	while (!is_broken_pipe)
	  {
	    ...
	    pixelWriter << heme::vis::controller->screen.pixels_x 
			<< heme::vis::controller->screen.pixels_y;
	
	    Network::send_all(new_fd, xdr_pixel, pixeldatabytes);

	    heme::io::XdrMemWriter pixelDataWriter = 
	      heme::io::XdrMemWriter(heme::vis::xdrSendBuffer_pixel_data,
			       heme::vis::pixel_data_bytes);
	    ...
	    for (int i = 0;
		 i < heme::vis::controller->col_pixels_recv[RECV_BUFFER_A];
		 i++) {
	      pixelDataWriter.writePixel(&heme::vis::controller->col_pixel_recv[RECV_BUFFER_A][i],
