...
const int D3Q15::INVERSEDIRECTIONS[] = { 0, 2, 1, 4, 3, 6, 5, 8, 7, 10, 9, 12,
                                         11, 14, 13 };

void D3Q15::CalculateDensityAndVelocity(const double f[],
                                        double &density,
                                        double &v_x,
                                        double &v_y,
                                        double &v_z)
...
void D3Q15::CalculateFeq(const double &density,
                         const double &v_x,
                         const double &v_y,
                         const double &v_z,
                         double f_eq[])
{
  ...
  f_eq[0] = (2.0 / 9.0) * density - (1.0 / 3.0) * ( (v_xx + v_yy + v_zz)
      * density_1);
  ...
  f_eq[7] = (temp1 + ( (1.0 / 16.0) * density_1) * temp2 * temp2) + ( (1.0
      / 24.0) * temp2); // (+1, +1, +1)
  f_eq[8] = (temp1 + ( (1.0 / 16.0) * density_1) * temp2 * temp2) - ( (1.0
      / 24.0) * temp2); // (-1, -1, -1)

  temp2 = (v_x + v_y) - v_z;

  f_eq[9] = (temp1 + ( (1.0 / 16.0) * density_1) * temp2 * temp2) + ( (1.0
      / 24.0) * temp2); // (+1, +1, -1)
  f_eq[10] = (temp1 + ( (1.0 / 16.0) * density_1) * temp2 * temp2) - ( (1.0
      / 24.0) * temp2); // (-1, -1, +1)

  temp2 = (v_x - v_y) + v_z;

  f_eq[11] = (temp1 + ( (1.0 / 16.0) * density_1) * temp2 * temp2) + ( (1.0
      / 24.0) * temp2); // (+1, -1, +1)
  f_eq[12] = (temp1 + ( (1.0 / 16.0) * density_1) * temp2 * temp2) - ( (1.0
      / 24.0) * temp2); // (-1, +1, -1)

  temp2 = (v_x - v_y) - v_z;

  f_eq[13] = (temp1 + ( (1.0 / 16.0) * density_1) * temp2 * temp2) + ( (1.0
      / 24.0) * temp2); // (+1, -1, -1)
  f_eq[14] = (temp1 + ( (1.0 / 16.0) * density_1) * temp2 * temp2) - ( (1.0
      / 24.0) * temp2); // (-1, +1, +1)
}
...
void D3Q15::CalculateDensityVelocityFEq(const double f[],
                                        double &density,
                                        double &v_x,
                                        double &v_y,
                                        double &v_z,
...
void D3Q15::CalculateVonMisesStress(const double f[],
                                    double &stress,
                                    const double iStressParameter)
{
  ...
  a = sigma_xx_yy * sigma_xx_yy + sigma_yy_zz * sigma_yy_zz + sigma_xx_zz
      * sigma_xx_zz;
...
// wall.
void D3Q15::CalculateShearStress(const double &density,
                                 const double f[],
                                 const double nor[],
                                 double &stress,
                                 const double &iStressParameter)
