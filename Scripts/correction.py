...
# Create a new checkpoint file from three existing files.
...
# ******************************************************************************
def createCorrectedDistributionFile(checkpointFileName1,
                                    checkpointFileName2,
                                    checkpointFileName3,
    ...
    sys.stderr.write("correctedCheckpointFileName: " + correctedCheckpointFileName + "\n")
    cwd = os.getcwd()
    sys.stderr.write("correction, cwd: " + cwd + "\n")
...
# ******************************************************************************
# Open a checkpoint file for writing. This will hold the corrected distributions.
...
# ******************************************************************************
    ...
    print "Creating checkpoint file", correctedCheckpointFileName
    dirName = os.path.dirname(correctedCheckpointFileName)
    if os.path.exists(dirName):
        sys.stderr.write('WARNING: ' + dirName + ' already exists but should not.\n')
    else:
        os.makedirs(dirName)
    correctedCheckpointFile = open(correctedCheckpointFileName, 'w')

    # Open a (checkpoint) offset file for writing.
    checkpointOffsetFileName = correctedCheckpointFileName[:-3] + "off"
    ...
    correctedCheckpointFile.write(p01.get_buffer())

    # Start packing the contents of the offset file.
    p01Off = xdrlib.Packer()
    p01Off.pack_uhyper(numProcs)
    p01Off.pack_uhyper(correctedCheckpointFile.tell())

    # Pack the time step.
    p02 = xdrlib.Packer()
    p02.pack_uhyper(fileInfo1.timestep)
    correctedCheckpointFile.write(p02.get_buffer())

# ******************************************************************************
# Decode the distributions that have been read and compute the corrected values.
# ******************************************************************************
    ...
    for r in range(numProcs-1):
        ...
        correctedCheckpointFile.write(p03.get_buffer())
    p01Off.pack_uhyper(correctedCheckpointFile.tell())
    checkpointOffsetFile.write(p01Off.get_buffer())
         
    correctedCheckpointFile.close()
