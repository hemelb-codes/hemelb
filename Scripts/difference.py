...
# Create a new checkpoint file which is the difference of two existing files.
...
# ******************************************************************************
# Open two checkpoint files for reading. The files must have a common structure.
...
# ******************************************************************************
def createDifferenceDistributionFile(checkpointFileName1,
                                    checkpointFileName3,
                                    differenceCheckpointFileName):

    sys.stderr.write("createDifferenceDistributionFile:args:\n")
    sys.stderr.write("checkpointFileName1: " + checkpointFileName1 + "\n")
    sys.stderr.write("checkpointFileName3: " + checkpointFileName3 + "\n")
    sys.stderr.write("differenceCheckpointFileName: " + differenceCheckpointFileName + "\n")
    ...
    # Open a checkpoint file for reading.
    print "Checkpoint file 3:", checkpointFileName3
    checkpointFile3 = open(checkpointFileName3, 'r')
...
# ******************************************************************************
    ...
    fileInfo3 = readCheckpointFilePreambleEtc(checkpointFile3, firstOffset, lastOffset)
...
# ******************************************************************************
    ...
    data3 = checkpointFile3.read(readLength)
    up05 = xdrlib.Unpacker(data3)

    checkpointFile3.close()

# ******************************************************************************
# Open a checkpoint file for writing. This will hold the difference of the
# distributions.
...
# ******************************************************************************
    ...
    print "Creating checkpoint file", differenceCheckpointFileName
    differenceCheckpointFile = open(differenceCheckpointFileName, 'w')

    # Open a (checkpoint) offset file for writing.
    checkpointOffsetFileName = differenceCheckpointFileName[:-3] + "off"
    ...
    differenceCheckpointFile.write(p01.get_buffer())

    # Start packing the contents of the offset file.
    p01Off = xdrlib.Packer()
    p01Off.pack_uhyper(numProcs)
    p01Off.pack_uhyper(differenceCheckpointFile.tell())

    # Pack the time step.
    p02 = xdrlib.Packer()
    p02.pack_uhyper(fileInfo1.timestep)
    differenceCheckpointFile.write(p02.get_buffer())

# ******************************************************************************
# Decode the distributions that have been read and compute the difference of the
# values.
# ******************************************************************************
    ...
    for r in range(numProcs-1):
        ...
        for n in range(nData):
            ...
            p03.pack_uint(y)
            p03.pack_uint(z)
            up05.unpack_uint() # Discard 3rd x value.
            up05.unpack_uint() # Discard 3rd y value.
            up05.unpack_uint() # Discard 3rd z value.
            for c in range(fileInfo1.numberOfComponents):
                distribution1 = up03.unpack_float()
                distribution3 = up05.unpack_float()
                distribution = distribution1 - distribution3
                p03.pack_float(distribution)
        differenceCheckpointFile.write(p03.get_buffer())
    p01Off.pack_uhyper(differenceCheckpointFile.tell())
    checkpointOffsetFile.write(p01Off.get_buffer())
         
    differenceCheckpointFile.close()
